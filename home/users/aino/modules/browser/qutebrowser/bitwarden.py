...
def pass_(domain):
    ...
    process = subprocess.run(
        ['bw', 'list', 'items', '--nointeraction', '--session', session_key] + ([] if domain is None else ['--url', domain]),
        capture_output=True,
    )

    err = process.stderr.decode(arguments.io_encoding).strip()
    if err:
        ...
        if "Vault is locked" in err:
            ...
            return pass_(domain, arguments.io_encoding, 0, arguments.password_prompt_invocation)

    if process.returncode:
        return []

    out = process.stdout.decode(arguments.io_encoding).strip()
...
def get_choice(candidates):
    ...
    return next((c for (i, c) in enumerate(candidates)
...
def main(arguments):
    ...
    subprocess.call(['keyctl', 'link', '@u', '@s'])
    ...
    for target in targets:
        target_candidates = pass_(target)
    ...
    showing_all = False
    if len(candidates) == 0:
        candidates = pass_(None)
        showing_all = True

    selection = get_choice(candidates)

    # Nothing was selected, simply return
    if not selection and not showing_all:
        candidates = pass_(None)
        selection = get_choice(candidates)
